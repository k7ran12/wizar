...
<body>
    <div class="container">
        ...
        <p>En <strong>{{ $documento->firma->comuna }}</strong>, a 29 de enero de 2023.</p>
        <div>
            <p class="sin_margin">Entre <strong>AUTOFACT CHILE SpA</strong> C.I. N° <strong>76.197.647-8</strong> domiciliado para estos efectos en <strong>Av. Providencia 2019, Oficina 41 A</strong>, COMUNA <strong>Providencia</strong>, como VENDEDOR Y <strong>{{ $documento->comprador->nombre }}</strong> C.I. N° <strong>{{ $documento->comprador->run }}</strong> quien compra para <strong>{{ $documento->comprador->nombre }}</strong> C.I. N° <strong>{{ $documento->comprador->run }}</strong> domiciliado para estos efectos en <strong>{{ $documento->comprador->direccion }}</strong>, COMUNA <strong>{{ $documento->firma->comuna }}</strong> como COMPRADOR se ha convenido el siguiente contrato de compraventa:</p>
            <p class="con_margin">PRIMERO. - <strong>AUTOFACT CHILE SpA</strong> vende a <strong>{{ $documento->comprador->nombre }}</strong> quien compra y acepta para <strong>{{ $documento->comprador->nombre }}</strong> <strong>{{ $documento->comprador->run }}</strong> un vehículo <strong>{{ $documento->vehiculo->tipo_vehiculo }}</strong>, con sus accesorios completos que el comprador declara haber recibido a su entera satisfacción, y después de haberlo revisado detenidamente por lo que se expresa no tener cargo alguno ni objeción de formular, declarando que renuncia expresamente a las acciones de saneamiento por vicios redhibitorios establecido en el párrafo 8 del Título XXIII del Libro IV del Código Civil. El vehículo se vende en el estado en que se encuentra y que el comprador conoce cabalmente.</p>
            <p class="con_margin">SEGUNDO. - El precio de venta es la suma de $«VALOR_VENTA» («VALOR_ESCRITO») que el comprador paga de la siguiente forma: $ «VALOR_VENTA» en EFECTIVO. Con una tasación fiscal de $ «VALOR_TASACION» («TASACION_ESCRITA»)</p>
            ...
            <p class="con_margin">SEXTO. - El vendedor declara expresa y juradamente, en forma individual, que ha firmado el presente instrumento, encontrándose físicamente dentro de los límites territoriales y geográficos de la comuna de <strong>Providencia</strong> Región <strong>Metropolitana</strong> lo cual le consta fehacientemente por conocer los límites de la comuna referida.</p>
            ...
            <p class="con_margin">SÉPTIMO. - El comprador declara expresa y juradamente, en forma individual, que ha firmado el presente instrumento, encontrándose físicamente dentro de los límites territoriales y geográficos de la comuna de <strong>{{ $documento->firma->comuna }}</strong> Región <strong>{{ $documento->firma->region }}</strong> lo cual le consta fehacientemente por conocer los límites de la comuna referida.</p>
            <p class="sin_margin">Esta declaración se realiza con el objeto de cumplir con lo dispuesto por el inciso final del artículo 400 del Código Orgánico de Tribunales al momento de la autorización de la firma del presente contrato.</p>
